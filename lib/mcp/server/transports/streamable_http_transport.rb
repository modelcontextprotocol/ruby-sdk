    ...
    module Transports
      class StreamableHTTPTransport < Transport
        ...
        def initialize(
          ...
          # Maps a `subscriptions/listen` request id to `{ stream: stream_object, filter: honored_subscription_filter }`
          # (SEP-2575). In-process only; a multi-worker deployment needs an external event bus to fan notifications out across processes,
        ...
        # response; an abrupt disconnect sends nothing. SSE keepalive pings are not sent yet.
